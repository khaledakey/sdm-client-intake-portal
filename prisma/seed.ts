...
async function main() {
  ...
  await prisma.documentRequest.createMany({
    data: [
      { businessId: business1.id, label: 'Logo files', documentType: 'LOGOS', required: true, note: 'High-resolution logo files (PNG, SVG, or AI/EPS).' },
      { businessId: business1.id, label: 'Brand guidelines', documentType: 'BRAND_GUIDELINES', required: false, note: 'Existing brand guidelines, if available.' },
      { businessId: business1.id, label: 'Website access information', documentType: 'WEBSITE_DOCUMENTS', required: true, note: 'CMS/hosting access details.' },
    ],
  });
  await prisma.activityLog.create({
    data: { businessId: business1.id, userId: client1.id, activityType: 'ACCOUNT_CREATED', description: 'Aoife Kelly created an SDM Client Portal account for Brightleaf Cafe.' },
  });
  ...
  const req2 = await prisma.documentRequest.create({
    data: { businessId: business2.id, label: 'Logo files', documentType: 'LOGOS', required: true, status: 'FULFILLED' },
  });
  ...
  await prisma.documentRequest.createMany({
    data: [
      { businessId: business2.id, label: 'Brand guidelines', documentType: 'BRAND_GUIDELINES', required: false },
      { businessId: business2.id, label: 'Website access information', documentType: 'WEBSITE_DOCUMENTS', required: true },
    ],
  });
  await prisma.activityLog.createMany({
    data: [
      { businessId: business2.id, userId: client2.id, activityType: 'ACCOUNT_CREATED', description: 'Darragh Byrne created an SDM Client Portal account for Kilkenny Fit Co.' },
      { businessId: business2.id, userId: client2.id, activityType: 'BUSINESS_INFO_UPDATED', description: 'Business information was updated.' },
      { businessId: business2.id, userId: client2.id, activityType: 'INTAKE_SUBMITTED', description: 'Marketing intake information was updated.' },
      { businessId: business2.id, userId: client2.id, activityType: 'DOCUMENT_UPLOADED', description: 'Uploaded "kfc-logo-pack.txt".' },
    ],
  });
  ...
  await prisma.activityLog.create({
    data: { businessId: business2.id, userId: client2.id, activityType: 'TICKET_CREATED', description: `Opened support ticket SDM-${ticket.ticketNumber}: Question about ad account access` },
  });
  ...
  const intake3 = await prisma.clientIntake.upsert({
  ...
  const req3a = await prisma.documentRequest.create({
    data: { businessId: business3.id, label: 'Logo files', documentType: 'LOGOS', required: true, status: 'FULFILLED' },
  });
  const req3b = await prisma.documentRequest.create({
    data: { businessId: business3.id, label: 'Website access information', documentType: 'WEBSITE_DOCUMENTS', required: true, status: 'FULFILLED' },
  });
  ...
  await prisma.activityLog.createMany({
    data: [
      { businessId: business3.id, userId: client3.id, activityType: 'ACCOUNT_CREATED', description: 'Niamh Walsh created an SDM Client Portal account for Cork Craft Brew Co.' },
      { businessId: business3.id, userId: client3.id, activityType: 'DOCUMENT_UPLOADED', description: 'Uploaded "cork-craft-brew-logo.txt".' },
      { businessId: business3.id, userId: admin.id, activityType: 'SDM_REVIEW_COMPLETED', description: 'Sinead Murphy completed the SDM review.' },
      { businessId: business3.id, userId: admin.id, activityType: 'ONBOARDING_COMPLETED', description: 'Sinead Murphy marked onboarding complete.' },
    ],
  });
