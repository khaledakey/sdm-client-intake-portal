...
- **Email is provider-agnostic** (`src/lib/email.ts`), controlled by `EMAIL_PROVIDER`: `console` (default, logs to stdout — local dev only), `smtp` (nodemailer — blocked on Railway Hobby/Trial), `webhook` (POSTs JSON to `EMAIL_WEBHOOK_URL`, meant for a Make.com scenario or Hostinger automation), or `resend` (the production transport — sends via the [Resend](https://resend.com) HTTPS API using `RESEND_API_KEY`/`EMAIL_FROM`/`EMAIL_REPLY_TO`). In production, `console` or a misconfigured `resend`/`webhook` provider logs a startup error and returns a `failed` result instead of silently no-op'ing, and no path ever logs a reset/verification/set-password token or URL outside local dev. `POST /api/users` reports the outcome in its response as `inviteEmail: { status: "sent" | "failed", messageId?, error? }`. Support-ticket creation emails both the client and `SDM_SUPPORT_INBOX`, with the ticket ID in the subject (`[SDM-1042] ...`), per the brief.
...
- `EMAIL_PROVIDER` / `RESEND_API_KEY` / `EMAIL_FROM` / `EMAIL_REPLY_TO` (or `SMTP_*` / `EMAIL_WEBHOOK_URL`) — wire up real email delivery. Set `EMAIL_PROVIDER=resend` in production.
...
**Has its own dedicated, active Make scenario** — `Integration Webhooks, Notion` — built separately (directly in Make) specifically for this form's flat payload shape, not the `dispatchIntegrationEvent()` envelope the rest of the app uses. `LEAD_INTAKE_WEBHOOK_URL` points at it. Its webhook trigger's declared field list matches this form's payload field-for-field (`event_type`, `submission_id`, `timestamp`, `form_version`, `first_name`, ... `marketing_opt_in`), and it does real dedup before writing to Notion: searches the Organisation by Submission ID, searches/dedupes the Contact by Email + Organisation relation, then creates or updates Organisation, Contact, and Opportunity records accordingly — with an error-notification email on any step failing.

An earlier revision of this form pointed at the `SDM Client Portal → Notion CRM Sync` webhook instead (see above) — that scenario is still inactive and built for the nested envelope shape, not this flat one, so it was never a working destination for `/get-started` submissions. If you're troubleshooting a lead that didn't arrive in Notion, check the `Integration Webhooks, Notion` scenario's execution history in Make first — not the CRM sync scenario above.
